...
export default function ListingsClient({
  ...
  }, [properties, selectedType, selectedArea, selectedStatus]);
      ...
      {filteredProperties.length === 0 && (
        <div className="text-center py-24 border border-dashed border-outline-variant my-12 bg-surface-container-low">
          ...
          <p className="font-label-caps text-on-surface-variant uppercase text-xs">
            {properties.length === 0
              ? 'There are currently no published listings available.'
          ...
          {properties.length > 0 && (
            <button
              ...
              className="mt-6 font-nav-link text-nav-link uppercase text-primary border-b border-primary pb-1 cursor-pointer"
            >
              Reset Filters
            </button>
          )}
